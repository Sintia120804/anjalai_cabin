    ...
    <div class="position-relative overflow-hidden text-center text-white min-vh-100 d-flex align-items-center justify-content-center"
        ...
        <div class="col-md-8 p-lg-5 mx-auto my-5 z-1">
            ...
            <h1 class="display-3 fw-bold mb-3 lh-tight" style="text-shadow: 0 4px 20px rgba(0,0,0,0.5);">
                ...
                <span class="text-primary"
                    style="background: -webkit-linear-gradient(45deg, #4e9af1, #0ea5e9); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Tanpa
                    Batas.</span>
            </h1>
            <p class="lead fw-light mb-5 fs-4 text-light opacity-75">
                Rasakan ketenangan Anjalai Cabin. Dirancang eksklusif untuk memberikan kenyamanan modern di tengah kesejukan
                alam tropis.
            </p>
            <div class="d-flex justify-content-center gap-3 mb-5">
                <a class="btn btn-primary btn-lg rounded-pill px-5 py-3 shadow-lg fw-bold" href="#cabins"
                    style="background: linear-gradient(135deg, #2563eb, #3b82f6); border:none;">
                    Eksplorasi Cabin
                </a>
                <a class="btn btn-outline-light btn-lg rounded-pill px-5 py-3 fw-medium backdrop-blur" href="#galeri"
            ...
            <div class="bg-white p-3 p-md-4 rounded-4 shadow-lg text-start mx-auto"
                style="max-width: 850px; backdrop-filter: blur(10px); background: rgba(255,255,255,0.95); transform: translateY(50px); transition: transform 0.3s ease;">
                <form action="{{ route('welcome') }}#cabins" method="GET">
                    <div class="row g-3 align-items-end">
                        <div class="col-12 col-md-5">
                            <label for="checkin" class="form-label text-dark fw-bold small"><i
                                    class="bi bi-calendar-check text-primary me-1"></i> Check-in</label>
                            <input type="date" class="form-control form-control-lg bg-light border-0 shadow-none text-dark"
                                id="checkin" name="checkin" value="{{ request('checkin') }}" required
                                min="{{ date('Y-m-d') }}">
                        </div>
                        <div class="col-12 col-md-5">
                            <label for="checkout" class="form-label text-dark fw-bold small"><i
                                    class="bi bi-calendar-x text-primary me-1"></i> Check-out</label>
                            <input type="date" class="form-control form-control-lg bg-light border-0 shadow-none text-dark"
                                id="checkout" name="checkout" value="{{ request('checkout') }}" required
                                min="{{ date('Y-m-d', strtotime('+1 day')) }}">
                        </div>
                        <div class="col-12 col-md-2">
                            <button type="submit"
                                class="btn btn-primary btn-lg w-100 fw-bold shadow-sm d-flex align-items-center justify-content-center"
                                style="background: linear-gradient(135deg, #2563eb, #3b82f6); border:none; height: 48px;">
        ...
        <div class="position-absolute bottom-0 w-100 overflow-hidden" style="line-height: 0;">
            <svg viewBox="0 0 1200 120" preserveAspectRatio="none" style="width: calc(100% + 1.3px); height: 80px;">
                <path
                    d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V120H0V95.8C59.71,118.08,130.83,119.93,197.36,97.77,256.47,78.29,310.87,60.84,321.39,56.44Z"
                    fill="#f8fafc"></path>
            </svg>
        </div>
    </div>
            ...
            <div class="row g-4 text-center">
                <div class="col-md-4">
                    <div class="p-4">
                        ...
                        <p class="text-muted">Jauh dari kebisingan kota, memberikan ketenangan mental yang maksimal di
                            tengah alam.</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="p-4">
                        ...
                        <p class="text-muted">Meskipun berada di tengah alam, fasilitas kabin tetap modern, bersih, dan
                            mewah.</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="p-4">
                        ...
                        <p class="text-muted">Nikmati pengalaman menginap premium dengan harga yang sangat bersaing dan
                            transparan.</p>
                    </div>
                </div>
            </div>
        ...
        <div class="container py-5">
            <div class="text-center mb-5">
                ...
                @if(request('checkin') && request('checkout'))
                    <div
                        class="d-inline-flex align-items-center bg-primary bg-opacity-10 text-primary px-4 py-2 rounded-pill border border-primary border-opacity-25 shadow-sm">
                        <i class="bi bi-calendar-check-fill me-2"></i>
                        <span>Tersedia untuk: <strong>{{ \Carbon\Carbon::parse(request('checkin'))->format('d M Y') }}</strong>
                            s/d <strong>{{ \Carbon\Carbon::parse(request('checkout'))->format('d M Y') }}</strong></span>
            ...
            <div class="row g-4">
                @forelse($cabins as $cabin)
                    <div class="col-md-6 col-lg-4">
                        <div class="card h-100 border-0 rounded-4 overflow-hidden cabin-card"
                            style="box-shadow: 0 10px 30px rgba(0,0,0,0.05); transition: transform 0.3s, box-shadow 0.3s;">
                            <div class="position-relative overflow-hidden" style="height: 250px;">
                                @if($cabin->galeris->count() > 0)
                                    <img src="{{ asset('storage/' . $cabin->galeris->first()->foto) }}"
                                        class="w-100 h-100 object-fit-cover cabin-img" alt="{{ $cabin->name_cabin }}">
                                @else
                                    <div
                            ...
                            <div class="card-body p-4 bg-white">
                                ...
                                <div class="d-flex justify-content-between align-items-center p-3 rounded-3 bg-light border">
                                    <div class="fs-5 fw-bold text-primary">Rp
                                        {{ number_format($cabin->harga_weekday, 0, ',', '.') }}<span
                                            class="fs-6 text-muted fw-normal">/mlm</span></div>
                                </div>
                            </div>
                            <div class="card-footer bg-white border-0 p-4 pt-0">
                                <a href="{{ route('cabin.show', ['cabin' => $cabin->id, 'checkin' => request('checkin'), 'checkout' => request('checkout')]) }}"
                                    class="btn btn-outline-primary w-100 rounded-pill py-2 fw-medium btn-hover-fill">
                                    Lihat Detail & Pesan
                                </a>
                            </div>
                        </div>
                ...
                @empty
                    <div class="col-12 text-center py-5">
                        <div class="p-5 bg-white rounded-4 shadow-sm border border-light">
                            ...
                            <p class="text-muted">Maaf, tidak ada unit yang tersedia untuk tanggal yang Anda pilih. Silakan coba
                                tanggal lain.</p>
                            <a href="{{ route('welcome') }}#cabins"
                                class="btn btn-primary rounded-pill px-4 mt-2 shadow-sm">Lihat Semua Cabin</a>
                        </div>
                    </div>
                @endforelse
            </div>
        ...
        <div class="container py-5">
            ...
            <div class="row g-4">
                @forelse($wahanas as $w)
                    <div class="col-md-4">
                        <div class="card h-100 border-0 bg-light rounded-4 overflow-hidden wahana-card"
                            ...
                            <a href="{{ route('wahana.show', $w->id) }}" class="text-decoration-none text-dark">
                                ...
                                <div class="card-body p-4 text-center">
                                    ...
                                    <div class="d-flex align-items-center justify-content-center gap-3 mb-3">
                                        ...
                                        @endif
                                        <span class="h5 fw-bold text-primary mb-0">IDR
                                            {{ number_format($w->harga / 1000, 0, ',', '.') }}K</span>
                                    </div>
                                    <p class="text-muted small mb-0 line-clamp-2">{{ $w->deskripsi }}</p>
                                </div>
                            </a>
                            <div class="card-footer bg-light border-0 p-4 pt-0 text-center">
                                <a href="{{ route('wahana.show', $w->id) }}"
                                    class="btn btn-outline-primary rounded-pill px-4 fw-medium w-100 btn-hover-fill">
                ...
                @forelse($galeriUmum as $g)
                    <div class="col-6 col-md-3">
                        <div class="position-relative overflow-hidden rounded-3 galeri-item"
                            ...
                            <img src="{{ asset('storage/' . $g->foto) }}" class="w-100 h-100 object-fit-cover" alt="Galeri"
                                style="transition: transform 0.4s;">
                            <div class="galeri-overlay position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center bg-dark bg-opacity-50"
    ...
    <div class="py-5" id="lokasi" style="background-color: #f8fafc;">
        <div class="container py-5">
            ...
            <div class="text-center mt-4">
                <p class="text-muted"><i class="bi bi-geo-alt-fill text-primary me-2"></i> Taluak anjalai, Kec. Lembah
                    Gumanti, Kabupaten Solok, Sumatera Barat 27371</p>
            </div>
        </div>
    </div>
